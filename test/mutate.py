...
TARGETS = [
    ...
    # #90. The two that matter most fail SILENT: an empty gap list hands over a tree whose hooks
    # are gone, and an empty hook set builds a cone without them — an unguarded Crawler either way.
    ("what a sparse tree lacks that its hooks need", "worktree.cone_gaps", "lib/showrunner/worktree.py",
     r"(def cone_gaps\(cfg, tree, base=\"HEAD\"\):\n)",
     "    return []\ndef _neutered_cone_gaps(cfg, tree, base=\"HEAD\"):\n"),
    ("the directories registered hooks exec from", "worktree.hook_dirs", "lib/showrunner/worktree.py",
     r"(def hook_dirs\(cfg, base=\"HEAD\"\):\n)",
     "    return set()\ndef _neutered_hook_dirs(cfg, base=\"HEAD\"):\n"),
    ("registered hooks with no directory in the finished tree", "worktree.hook_gaps",
     "lib/showrunner/worktree.py",
     r"(def hook_gaps\(cfg, tree\):\n)",
     "    return []\ndef _neutered_hook_gaps(cfg, tree):\n"),
    ("refusing a spawn whose leaf a live process holds", "cli._live_claim_check",
     "lib/showrunner/cli.py",
     r"(def _live_claim_check\(leaf\):\n)",
     "    return None\ndef _neutered_live_claim_check(leaf):\n"),
    ("whether a LIVE Crawler stands in a tree", "worktree.live_crawler_in",
     "lib/showrunner/worktree.py",
     r"(def live_crawler_in\(cfg, name\):\n)",
     "    return None\ndef _neutered_live_crawler_in(cfg, name):\n"),
    ("a Crawler's shell destruction of scratch it does not own", "lease.scratch_destruction",
     "lib/showrunner/lease.py",
     r"(def scratch_destruction\(cfg, session, command, cwd=None\):\n)",
     "    return []\ndef _neutered_scratch_destruction(cfg, session, command, cwd=None):\n"),
    ("the paths a destructive argv would destroy", "lease._destroyed_operands",
     "lib/showrunner/lease.py",
     r"(def _destroyed_operands\(argv\):\n)",
     "    return []\ndef _neutered_destroyed_operands(argv):\n"),
    ("each shell command's argv, quotes resolved", "lease._argvs", "lib/showrunner/lease.py",
     r"(def _argvs\(command\):\n)",
     "    return []\ndef _neutered_argvs(command):\n"),
    ("what the default branch did to a leaf's paths since its base", "worktree.drift_report",
     "lib/showrunner/worktree.py",
     r"(def drift_report\(cfg, leaf, base_sha\):\n)",
     "    return {\"ref\": None, \"merge_base\": None, \"deleted\": [], \"modified\": [], "
     "\"unknown\": None}\ndef _neutered_drift_report(cfg, leaf, base_sha):\n"),
    ("the refusal of a base whose leaf paths were deleted", "cli._base_drift_check",
     "lib/showrunner/cli.py",
     r"(def _base_drift_check\(leaf, drift, despite, rehearsing=False\):\n)",
     "    return None\ndef _neutered_base_drift_check(leaf, drift, despite, rehearsing=False):\n"),
    ("what the brief tells a Crawler about deleted paths", "brief._drift_block",
     "lib/showrunner/brief.py",
     r"(def _drift_block\(spawn_record\):\n)",
     "    return \"\"\ndef _neutered_drift_block(spawn_record):\n"),
    ("whether an inject path is inside a sparse cone", "worktree.in_cone",
     "lib/showrunner/worktree.py",
     r"(def in_cone\(path, cone\):\n)",
     "    return True\ndef _neutered_in_cone(path, cone):\n"),
    ("declared paths outside a leaf's cone", "worktree.cone_misses", "lib/showrunner/worktree.py",
     r"(def cone_misses\(leaf, cone\):\n)",
     "    return []\ndef _neutered_cone_misses(leaf, cone):\n"),
    ("what the brief tells a Crawler about its cone", "brief._cone_block", "lib/showrunner/brief.py",
     r"(def _cone_block\(spawn_record\):\n)",
     "    return \"\"\ndef _neutered_cone_block(spawn_record):\n"),
